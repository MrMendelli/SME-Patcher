...
echo.
%cecho% {\u07 \u07}
%cecho% {0c}MD5 checksum mismatch! The MD5 does not match the required checksum:{\n}
echo.
%cecho% {0e}%GameTitle%: {0a}%VanillaChecksum%{\n}
%cecho% {07}%~nx1 {0e}: {0c}%MD5%{\n}
echo.
...
cls
echo.
%cecho% {0a}MD5 checksum match! The MD5 matches the required checksum:{\n}
echo.
%cecho% {0e}%GameTitle%: {0a}%VanillaChecksum%{\n}
%cecho% {07}%~nx1 {0e}: {0a}%MD5%{\n}
...
echo.
"%~dp0bin\xdelta.exe" -d -f -s "%~1" "%PatchFile%.xdelta" "%~dp1(%ModID%) %ModTitle% %PatchFile%.%OutFormat%"
%cecho% {02}Patching completed successfully! Press any key to view patched file.{\n}
pause > nul
start "" explorer /select, "%~dp1(%ModID%) %ModTitle% %PatchFile%.%OutFormat%"
exit/b
...
echo.
%cecho% {0e}%ModAlias% %ModVersion%: {0a}%ModChecksum%{\n}
%cecho% {07}%~nx1 {0e}: {0a}%MD5%{\n}
echo.
%cecho% {0e}
set /p choice="Update to %ModTitle% %UpdateFile%?: "
...
:ApplyUpdate
echo.
%cecho% {0e}Patching {07}%~nx1{0e} with {07}%ModTitle% %UpdateFile%{0e}...{\n}
echo.
"%~dp0bin\xdelta.exe" -d -f -s "%~1" "%~dp0patches\%UpdateFile%.update" "%~dp1(%ModID%) %ModTitle% %UpdateFile%.%OutFormat%"
%cecho% {02}Patching completed successfully! Press any key to view patched file.{\n}
pause > nul
start "" explorer /select, "%~dp1%(%ModID%) ModTitle% %UpdateFile%.%OutFormat%"
exit/b
